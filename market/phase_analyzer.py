...
class MarketPhaseAnalyzer:
    ...
    def identify_market_phases (self,btc_data: pd.DataFrame) -> pd.Series:
        ...
        combined_phases=pd.Series (index=phases.index)

        # Combinaison efficace des phases et niveaux de volatilité
        for date in phases.index:
            phase=phases.loc[date]
            vol_suffix='_high_vol' if high_volatility.loc[date] else '_low_vol'
            combined_phases.loc[date]=phase + vol_suffix
